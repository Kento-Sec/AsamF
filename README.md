...
对于windows用户非常抱歉，出现了一些问题。

修复了windows的一些问题，如果并不是第一次使用，已经存在config.json文件，请在c:\Users\用户名\目录下，手动新建asamf的文件夹即可修复搜索结果自动保存的问题。

如果没有这个config.json文件，在首次运行的时候在生成配置文件的同时，会在c:\Users\用户名下新建asamf文件夹，用于存储搜索结果
